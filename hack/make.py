...
def fmt():
    libbuild.ungroup_go_imports('cmd.go', 'main.go', 'mount.go', 'version.go', 'volume')
    die(call('goimports -w *.go volume'))
    call('gofmt -s -w *.go volume')


def vet():
    call('go vet *.go volume')


def lint():
    call('golint *.go volume')
